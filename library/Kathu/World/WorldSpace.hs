...
emptyWorldSpace = WorldSpace "" "the void..." "" Map.empty (V2 0 0) Map.empty Vec.empty Vec.empty Map.empty
...
initWorldSpace :: forall w m g. (MonadIO m , Get w m EntityCounter, Get w m (Tiles g), Has w m Physics, ReadWriteEach w m '[Existance, Local, LifeTime, Render g, WorldSpace g, WorldStases])
               => (Entity -> SystemT w m ())
               -> (EntityPrototype g -> SystemT w m Entity)
               -> WorldSpace g
               -> SystemT w m ()
initWorldSpace destroyEty mkEntity ws = do
...
instance ( s `CanProvide` (IDMap (EntityPrototype g))
    ...
    parseJSON (Object v) = do
        ...
        pure $ WorldSpace worldId wName initPal palettes loadPnt variables <$> entities <*> items <*> newFieldSet
