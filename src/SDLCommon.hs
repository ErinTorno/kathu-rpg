...
withWindow :: MonadIO m => Text -> SDL.V2 Word32 -> (SDL.Window -> m a) -> m ()
withWindow title (SDL.V2 w h) op = do
    window <- SDL.createWindow title $ SDL.defaultWindow {SDL.windowInitialSize = SDL.V2 (fromIntegral w) (fromIntegral h)}
